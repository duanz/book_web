...
class Image(BaseModel):
    ...
    origin_addr = models.CharField("原始地址", max_length=200, unique=True, default="")
...
class Book(BaseModel):
    ...
    book_type = models.CharField(
        "书本类型", max_length=30, default="", choices=BOOK_TYPE_CHOICES
    )
    ...
    def latest_chapter(self):
        chapter = (
            Chapter.normal.filter(book_id=self.id, book_type=self.book_type)
            .order_by("-create_at")
            .first()
        )
        return chapter
...
class Chapter(BaseModel):
    ...
    def save_content(self, content):
        ...
        if self.book_type == BOOK_TYPE_DESC.Novel:
            ...
            cis = [
                ChapterImage(
                    book=self.book, chapter_id=self.id, image=image, order=index
                )
    ...
    def content(self):
        ...
        if self.book_type == BOOK_TYPE_DESC.Novel:
            ...
            img_ids = ChapterImage.normal.filter(
                book=self.book, chapter=self.pk
            ).values_list("image")
