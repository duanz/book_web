...
class AutoInsertBookClient(BaseClient):
    ...
    def book_insert(self, url):
        ...
        bic = BookInsertClient(url, parser_cls.book_type, "with_content", True, True)
        bic.run()
        lock.acquire()
        self.current_run_threading.remove(url)
        logging.info("当前还有线程 共 {} 条等待执行结束".format(len(self.current_run_threading)))
        lock.release()
    ...
    def handler(self):
        ...
        for site in parser_selector.regular.keys():
            if not hasattr(parser_selector.get_parser(site), "all_book_url_one_by_one"):
                continue
        ...
        for site in parser_selector.regular.keys():
            ...
            if parser_cls.total_page > together_num:
                ...
                while times * together_num < parser_cls.total_page:
                    end_num = (
                        parser_cls.total_page
                        if (parser_cls.total_page - times * together_num) < together_num
                        else (times + 1) * together_num
                    )
