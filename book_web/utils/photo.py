            ...
            if width_scale > 1 and height_scale > 1:
                if width_scale > height_scale:
                    size_box = (
                        ...
                        0,
                        int((photo_width - photo_height) / 2) + photo_height,
                        photo_height,
                    )
                else:
                    size_box = (
                        ...
                        photo_width,
                        photo_width + int(photo_height - photo_width) / 2,
                    )
                ...
                if width_scale > height_scale:
                    size_box = (
                        int((photo_width - photo_height) / 2),
                        0,
                        int((photo_width - photo_height) / 2) + photo_width,
                        photo_height,
                    )
...
def split_photo_fit_kindle(photo_file, outpath=settings.MEDIA_ROOT):
    ...
    if img_split_num == 1:
        ...
        for i in range(img_split_num):
            shape = (
                0,
                int(orig_width * 1.2 * i),
                orig_width,
                int(orig_width * 1.2 * (i + 1)),
            )
            new_img = img.crop(shape)
            filename = os.path.join(
                outpath, "{}_{}".format(i, os.path.split(photo_file)[-1])
            )
...
def save_upload_photo(
    photo_file,
    base_static_path=settings.MEDIA_ROOT,
    photo_type="photo",
    extension=".jpg",
):
    ...
    d_path_raw = os.path.join(
        base_static_path, photo_type_dict[photo_type]["sub_path"], "raw", photo_id[:2]
    )
...
def get_img_url_by_obj(img_obj, pic_type="photo", ext="jpg"):
    ...
    if isinstance(img_obj, dict):
        return build_photo_url(
            photo_name=img_obj.get("key"), pic_type=pic_type, ext=ext
        )
